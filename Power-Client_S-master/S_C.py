...
def login():
    if request.method == 'POST':
        ...
        cur.close()
        if len(user)>0:     
            sPass=hashlib.sha512(password.encode('utf-8')  + seed.encode('utf-8')).hexdigest()
            if sPass == user[3]:                
                session['name']=user[1]
                #print(session['name'])
...
def prueba():
    ...
    return render_template('prueba.html', tickets = tickets,tta= _ticketactivos[0][0],ttt= _tickettotal[0][0],ttf= _ticketfinal[0][0], nomre= _username[0][0])
...
def calificar():
    if request.method == 'POST':
        Modelo.pasos(session['name'],'CALIFICAR', 'USUARIO CALIFICA UN ASESOR')
        data=request.get_json()
